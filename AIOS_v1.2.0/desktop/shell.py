...
class AiosShell(QMainWindow):
    ...
    def _make_command_center(self) -> QFrame:
        ...
        voice_btn.setToolTip("Record 5 seconds and transcribe locally with Vosk")
        voice_btn.clicked.connect(self._start_voice_input)
        media.addWidget(voice_btn)
        vision_btn = QPushButton("◉ Capture Screen")
        vision_btn.setObjectName("mediaButton")
        vision_btn.setToolTip("Capture the screen for the AIOS vision layer")
        vision_btn.clicked.connect(self._capture_screen)
        media.addWidget(vision_btn)
        vision_status = self.voice_engine.status()
        voice_state = "READY" if vision_status.get("ready") else "OPTIONAL SETUP"
        self.media_status = QLabel(f"VOICE: {voice_state}   •   VISION: READY")
        self.media_status.setObjectName("mediaStatus")
        media.addWidget(self.media_status)
        media.addStretch(1)
    ...
    def _capture_screen(self) -> None:
        ...
        if result.ok:
            ...
            self.workspace_status.setText("VISION UNAVAILABLE")
    ...
    def _start_voice_input(self) -> None:
        if self._voice_thread is not None and self._voice_thread.isRunning():
            ...
            return
        self.output.setText("● Listening for 5 seconds…")
